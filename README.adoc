...
#Lohan# (et #Kristen éventuellement à distance quand il reviendra#)

Commentaires éventuels des étudiants : #Kristen : Pas (encore) là#
...
Ce code a une stucture très basique, ce qui n'est pas une structure en couche.

*Où se trouve le code métier (voir la règle de gestion RG 1) ?*
Il se trouve dans le controller de TodoList

*Cette règle est-elle facilement testable par un test unitaire ?*
Elle est testable mais Spring complexifie la tâche comparé  au Java natif. Il faut bien tester en dehoors ddu controlleur.
...
La commande permet de lancer un conteneur Docker. -it permet de le lancer en interactif. -e permet de mettre une variable d'environnement, et -p permet de renseigner le numéro de port.

* Compléter le code manquant dans la méthode `TodoListController.createTodoItem()`

*Pourquoi `todoItemRepository` est-il `null` ? Quelle est la meilleure façon de l'injecter ?
TodoItemRepository est null car il n'a pas été injecté. Il faut rajouter la mention @Inject au dessus du constructeur
